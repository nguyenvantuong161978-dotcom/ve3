...
class GoogleFlowAPI:
    ...
    def _poll_proxy_video_task(
        ...
        # STEP 1: Poll proxy to get operations array — chờ vô hạn
        operations = None
        start_time = time.time()
        ...
        while True:
            ...
            try:
                ...
                result = response.json()
                if attempt % 6 == 1:  # Log mỗi ~30s
                    self._log(f"Video poll {attempt}: ({int(elapsed)}s) {json.dumps(result)[:200]}")
                ...
                if not result.get("success"):
                    ...
                    # Chưa xong, đang chờ — update last_status_change
                ...
                if operations:
                    self._log(f"Got operations from proxy ({int(elapsed)}s), switching to Google polling...")
                    break

                # Có thể server đã xong và trả kết quả trực tiếp (video URL)
                # Check nếu result có video data
                last_status_change = time.time()
                time.sleep(5)
